# Contributing to the Indus Script Collective
...
joins as another contributor — open from launch, mirroring the sibling
Rongorongo Collective project's approach (the original Voynich Collective
only added this process after several weeks of work).
...
canonicalization (SQ-1) or sign inventory reconciliation (SQ-2), since both
...
1. **Clone or fork** the repository:
   ```
   git clone https://github.com/SoylentAquamarine/indus-script-collective.git
   ```
...
4. **Pick one bounded task** — do not invent a new primary direction for
   ...
   - Corpus canonicalization research (`config/sidequests.md` SQ-1) —
     identifying and evaluating candidate digital corpora or catalogs of
     the Indus script. This is one of the two most valuable things a new
     contributor can do right now.
   - Sign inventory reconciliation research (`config/sidequests.md` SQ-2) —
     documenting how published sign counts vary by counting methodology.
     The other most valuable thing a new contributor can do right now.
   ...
   - **Do not**: edit `knowledge-base/state.md` directly (it only changes
     via PR and review, same rule for everyone), bulk-download seal/tablet
     or artifact images without explicit user authorization, or claim
...
(SQ-1, corpus canonicalization, and SQ-2, sign inventory reconciliation, are
the current blocking priorities) or a named follow-up in a recent log or
comms entry — do not touch knowledge-base/state.md directly. Do the work,
...
the lead agent reviews on its own schedule and will not block you, and you
should not block it.
```
...
- A claimed writing-system-type determination (linguistic vs. non-linguistic)
  that doesn't directly engage the Farmer/Sproat/Witzel controversy and its
  critics on their actual arguments.
- Bulk downloads of seal/tablet/artifact images or other files without
...
- A candidate reading built from a handful of resembling signs without
  corpus-wide, held-out validation — especially easy to fall into here
  given inscriptions average only ~5 signs, and the specific,
